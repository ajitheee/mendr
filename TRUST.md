...
Status: current for `v0.5.5-alpha` and `main`. The *Provider SDKs* row (in the
human report, and in the job summary through `MENDR_JOB_SUMMARY`, section 2) and
the *Python SDKs* row (root `requirements*.txt` and `uv.lock`) are new in
`v0.5.4-alpha`; no earlier release has them. Anything marked *planned* does not
