...
Want the read-only version first? `npx github:ajitheee/mendr#v0.4.5-alpha audit . --install` scaffolds a workflow that keeps one tracking issue current and changes no code — `contents: read`, `issues: write`, nothing else. This action is the step after that: it opens the verified migration as a PR.
...
| `mendr-spec` | `github:ajitheee/mendr#v0.4.5-alpha` | the CLI that runs in your CI (npm spec once published) |
